...
# cd $workdir
# python dataset_tool.py create_from_images_with_labels $our_data_path/Tfrecord256Label $our_data_path/CropImgJpg --label_names 'NF1Before,NF1After,NF1Inter' --resolution 256 
...
--data=$our_data_path/Tfrecord256Label \
--gpus=2 \
--mirror=1 \
--aug=ada --target=0.7 \
--augpipe=bgc \
--metrics=kid50k_full,fid50k_full \
--outdir=$our_data_path/Model \
--resume=ffhq256 \
--cfg=CONFIG \
--intermediate_state 
...
for CONFIG in ['paper256'] : 
  ...
  newscript = re.sub('WORKDIR',str(workdir),script)
