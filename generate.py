...
def generate_images(network_pkl, seeds, truncation_psi, outdir, class_idx, dlatents_npz, mix_ratio=0.25, class_idx_next=None):
    tflib.init_tf()
    ...
    if (class_idx is not None) and (class_idx_next is None):
        label[:, class_idx] = 1 # ! takes in label, but @class_idx is int, so we pass in just 1 label at a time.

    if class_idx_next is not None: # ! mix labels. LabelEmb is a linear layer in the format LinearWeight * Y, where Y is one-hot
        label[:, class_idx] = mix_ratio # 0-->after, 1-->before
        label[:, class_idx_next] = 1-mix_ratio
...
def main():
    ...
    parser.add_argument('--class_next', dest='class_idx_next', type=int, help='next label to morph')
